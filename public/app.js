...
// ─── CONFIG — Usando API local ───────────────────────────────────────────────
const API_MACHINES = '/api/v2/machines';
const API_RECORDS = '/api/v2/records';
const API_HISTORY = '/api/v2/history';
// ─────────────────────────────────────────────────────────────────────────────
...
async function loadMachines(preselect = null) {
  ...
  if (true) { // Usar siempre API
    try {
      const res = await fetch(API_MACHINES, { cache: 'no-store' });
...
async function submitToSheets() {
  const operarioPin = document.getElementById('operarioPin').value.trim();

  // Validación básica del PIN
  if (!operarioPin || operarioPin.length < 4) {
    showToast('Por favor, introduce tu PIN de operario válido', 'error');
    return;
  }
  ...
  try {
    ...
    successTitle.style.display = '';
    btnNewRecord.style.display = '';
    successDetail.textContent = `Registro enviado correctamente.`;
  } catch (error) {
    if (error.message.includes('PIN')) {
      ...
      successTitle.style.display = '';
      btnNewRecord.style.display = '';
      successDetail.textContent = `Error: ${error.message}. Guardado localmente.\nSe intentará reenviar.`;
      flushOfflineQueue();
    }
  } finally {
...
function resetApp() {
  ...
  // Reset UI inputs
  notes.value = '';
  const pinInput = document.getElementById('operarioPin');
...
async function fetchGlobalHistory() {
  ...
  try {
    ...
    if (json.ok) {
      if (!json.data) {
        throw new Error("El backend no devolvió el historial.");
      }
      globalHistoryCache = json.data || [];
      renderGlobalHistory();
    } else {
